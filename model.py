...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    for idx, token in enumerate(specials): # Assigning ids to special tokens first
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    if len(ids) < max_len:
        num_padding_tokens = max_len - len(ids)
        return ids + [pad_id] * num_padding_tokens
    else:
        return ids[:max_len]
...
def compute_positional_div_term(d_model):
    ...
    positions = torch.arange(0, d_model, 2).float()
    div_term = torch.exp(positions * (-math.log(10000.0) / d_model))
    return div_term
